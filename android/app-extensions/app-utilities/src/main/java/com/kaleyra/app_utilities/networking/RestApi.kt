...
class RestApi(val applicationContext: Context) {
    ...
    fun getAccessTokenNew(userId:String, apiKey:String, env:String, region:String, onSuccess: (String) -> Unit, onError: (Exception) -> Unit) {
        scope.launch {
//            updateConfiguration()
            kotlin.runCatching {
                val endPoint = "https://cs$env.${region}.bandyer.com"
                Log.d("url called", endPoint)
                val response: HttpResponse = client.post("$endPoint/rest/sdk/credentials") {
                    headers({
                    append("apiKey", apiKey)
                })
                ...
                Log.e("apiKey", apiKey)
                Log.e(TAG, response.body<AccessToken.Response>().access_token)
                currentAccessToken = response.body<AccessToken.Response>().access_token
            }.onFailure {
                Log.e(TAG, it.message ?: "Unable to fetch access token")
                withContext(Dispatchers.Main) {
